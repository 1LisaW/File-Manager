...
export const decompressFile = async (args) => {
    ...
    if (sourceExtname !== '.gz') return () => 'Operation failed';
    ...
    try {
        ...
        if (isDirectory) {
            const uncompressedFileName = sourceFileName.endsWith('.gz') ? sourceFileName.substring(0, sourceFileName.length - 3) : sourceFileName;
